...
using EnergyMonthDataHandle = WebReport.Dal.EnergyMonthDataHandle;
using EnergyType = WebReport.DataContact.EnergyType;
using ExceptionMsg = WebReport.ResultView.ExceptionMsg;
using IEnergyMonthDataBll = WebReport.Interface.IEnergyMonthDataBll;
using PageInfo = WebReport.Model.PageInfo;
using QueryEnergyStatistic = WebReport.DataContact.QueryEnergyStatistic;
using ResultEnergMonthyDataList = WebReport.ResultView.ResultEnergMonthyDataList;
using ResultEnergyMonthData = WebReport.ResultView.ResultEnergyMonthData;
...
namespace WebReport.Bll
{
    ...
    public class EnergyMonthDataBll : IEnergyMonthDataBll
    {
        ...
        public string GetEnergyDataList(QueryEnergyStatistic queryObj)
        {
            ...
            var result = new ResultEnergMonthyDataList {DevList = new List<ResultEnergyMonthData>()};
            try
            {
                ...
                result.DevList = (result.DevList.Skip(obj.Page.CurrPage*int.Parse(obj.Page.NumAPage))).Take(int.Parse(obj.Page.NumAPage)).ToList();
            }
            catch (Exception ed)
            {
        ...
        public string GetPageInfo(QueryEnergyStatistic queryObj)
        {
            ...
            try
            {
                ...
                result2.Page.Num = (result2.Page.Num / int.Parse(obj.Page.NumAPage) >= 0 && result2.Page.Num % int.Parse(obj.Page.NumAPage) > 0 ? result2.Page.Num / int.Parse(obj.Page.NumAPage) + 1
                    : result2.Page.Num / int.Parse(obj.Page.NumAPage));
        ...
        public string ExportExcel()
        {
            ...
            var result1 = new ResultEnergMonthyDataList { DevList = new List<ResultEnergyMonthData>() };
            var query = _context.Request.Form["inputs"];
            try
            {
