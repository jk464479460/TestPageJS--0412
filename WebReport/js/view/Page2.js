...
var cookie = getCookie('admin');
    ...
    $('#timeType').change(function() {
        ...
        if (select == 1) {
            $('#endDate').parent().css({ "display": "none" });

           
        } else {
            $('#endDate').parent().css({ "display": "inline" });
...
Page1.prototype = {
    ...
    url: function () {
        return {
            ...
            GetEnergyMonthDataList: "../Service/EnergyMonthDataService.svc/GetEnergyDataList",
            GetPageInfo: "../Service/EnergyMonthDataService.svc/GetPageInfo",
            //UpdateInsertEnergyData: "../Service/EnergyDataService.svc/UpdateInsertEnergyData",
            ExportExcel: "../Service/EnergyMonthDataService.svc/ExportExcel"
        };
    },
    ...
    GetQuerJson: function (currNum) {
        ...
        if (timeType == 2) {
            ...
            var ctime = startTime.replace(/-/g,"/");//为了兼容IE
            var newDate = new Date(ctime);
            newDate.setHours(newDate.getHours()+1);
            var tempDay = newDate.getFullYear();
            var month = parseInt(newDate.getMonth()) + 1;
            var day = newDate.getDate();
            
            var h = newDate.getHours();
            queryData.EndTime =tempDay +"-"+month+"-"+day
                +" "+h+":00:00";
        }
    ...
    GetQueryExport:function() {
        ...
        var energyType = $("#energyType").val();
        dataStr = {
            DevNum:"1", "Cname":"", TimeType:timeType, "StartTime":startTime,
            "EndTime":"", EnergyType:energyType
        };
    },
    ExcelExport: function () {
        ...
        $.ajax({
            ...
            data: iputStr,//{ "Inputs":  iputStr},//iputStr
            success: function (data, textStatus) {
                var obj = JSON.parse(data.d);
                if (obj.Exception.Success) {
                    alert(90);
    ...
    QueryEnergyData: function (currNum) {
        ...
        $.ajax({
            url:  "../Service/EnergyMonthDataService.svc/GetEnergyDataList",
            type: "POST",
