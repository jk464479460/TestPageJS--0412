...
namespace ServiceLib.Bll
{
    ...
    public class EnergyDataBll : IEnergyDataBll
    {
        ...
        public string GetEnergyDataList()
        {
            var query = _context.Request.Form["inputs"];
            var result = new ResultView.ResultEnergyDataList();
            try
            {
                ...
                var i = 0;
                var baseId = obj.Page.NumAPage*(obj.Page.CurrPage-1);
                foreach (Model.EnergyData p in res)
                {
        ...
        public string GetPageInfo()
        {
            ...
            try
            {
                ...
                result.Page.Num = (result.Page.Num / obj.Page.NumAPage >= 0 && result.Page.Num %obj.Page.NumAPage>0 ? result.Page.Num / obj.Page.NumAPage + 1
                    : result.Page.Num/obj.Page.NumAPage);
                result.Exception = new ExceptionMsg { Exsg = "", Success = true };
            }
            catch (Exception ed)
            {
        ...
        public string UpdateInsertEnergyData()
        {
            var result = new ReusltSavingData();
            var query = _context.Request.Form["inputs"];
            try
            {
                var obj = JsonConvert.DeserializeObject<QueryUpdateData>(query);
                var devNum = obj.DevNum;
                ...
                var timeidStr = new Tuple<string, string>("#timeid#",obj.TimeId.ToString("yyyy-MM-dd HH:mm:ss"));
